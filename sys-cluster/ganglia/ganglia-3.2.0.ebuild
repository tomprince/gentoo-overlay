...
inherit eutils multilib webapp python distutils
...
src_prepare() {
	...
	epatch ${FILESDIR}/gmetad-python-pid.patch
	if use python; then
		( cd gmetad-python && distutils_src_prepare )
	fi
}
...
src_compile() {
	default_src_compile
	( cd gmetad-python && distutils_src_compile )
}
...
src_install() {
	...
	if use python; then
		( cd gmetad-python && distutils_src_install )
		newinitd "${FILESDIR}"/gmetad-python gmetad-python
	fi
...
pkg_postinst() {
	...
	( use python && cd gmetad-python && distutils_pkg_postinst )
}
